...
if(isset($_GET['add'])){
  ...
  while($row = fetch_array($query)){
    ...
    if($row['product_quantity'] != $_SESSION['product_'.$_GET['add']]){
      ...
      set_message("Only ".$row['product_quantity']." ".$row['product_title']." available");
      redirect("../public/checkout.php");
    }
  }
}
    ...
    foreach ($_SESSION as $name => $value) {
      ...
      if($value > 0){
        if(substr($name, 0, 8) == "product_"){
          ...
          values ('{$price}','{$transaction}','{$status}','{$currency}')  ");
          $last_id = last_id();
          confirm($send_order);
          ...
          while($row = fetch_array($query)){
            ...
            $quer = query("INSERT INTO reports(product_id, product_title, order_id, product_price, product_quantity) values('{$id}', '{$row['product_title']}', '{$last_id}', '{$row['product_price']}', '{$value}')  ");
            confirm($quer);
          } //END OF WHILE
